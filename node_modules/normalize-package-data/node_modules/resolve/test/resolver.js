...
test('bar', function (t) {
    ...
    resolve('foo', { basedir: dir + '/bar' }, function (err, res, pkg) {
        if (err) t.fail(err);
        t.equal(res, path.join(dir, 'bar/node_modules/foo/index.cjs'));
...
test('biz', function (t) {
    ...
    resolve('./grux', { basedir: dir }, function (err, res, pkg) {
        if (err) t.fail(err);
        t.equal(res, path.join(dir, 'grux/index.cjs'));
        t.equal(pkg, undefined);
    });
    ...
    resolve('./garply', { basedir: dir }, function (err, res, pkg) {
        if (err) t.fail(err);
        t.equal(res, path.join(dir, 'garply/lib/index.cjs'));
    ...
    resolve('tiv', { basedir: dir + '/grux' }, function (err, res, pkg) {
        if (err) t.fail(err);
        t.equal(res, path.join(dir, 'tiv/index.cjs'));
        t.equal(pkg, undefined);
    });
...
test('quux', function (t) {
    ...
    resolve('./foo', { basedir: dir, 'package': { main: 'quux' } }, function (err, res, pkg) {
        if (err) t.fail(err);
        t.equal(res, path.join(dir, 'foo/index.cjs'));
...
test('normalize', function (t) {
    ...
    resolve('../grux', { basedir: dir }, function (err, res, pkg) {
        if (err) t.fail(err);
        t.equal(res, path.join(dir, 'index.cjs'));
        t.equal(pkg, undefined);
    });
});
...
test('#52 - incorrectly resolves module-paths like "./someFolder/" when there is a file of the same name', function (t) {
    ...
    resolve('./foo/', { basedir: path.join(dir, 'same_names') }, function (err, res, pkg) {
        if (err) t.fail(err);
        t.equal(res, path.join(dir, 'same_names/foo/index.cjs'));
    });
});
...
test('async dot main', function (t) {
    ...
    resolve('./resolver/dot_main', function (err, ret) {
        t.notOk(err);
        t.equal(ret, path.join(__dirname, 'resolver/dot_main/index.cjs'));
...
test('async dot slash main', function (t) {
    ...
    resolve('./resolver/dot_slash_main', function (err, ret) {
        t.notOk(err);
        t.equal(ret, path.join(__dirname, 'resolver/dot_slash_main/index.cjs'));
        t.ok(new Date() - start < 50, 'resolve.sync timedout');
        t.end();
    });
});
