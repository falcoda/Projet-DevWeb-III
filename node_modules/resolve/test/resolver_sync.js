...
test('bar', function (t) {
    ...
    t.equal(
        ...
        path.join(dir, 'bar/node_modules/foo/index.cjs')
    );
    t.end();
});
...
test('biz', function (t) {
    ...
    t.equal(
        ...
        path.join(dir, 'grux/index.cjs')
    );

    t.equal(
        resolve.sync('tiv', { basedir: path.join(dir, 'grux') }),
        path.join(dir, 'tiv/index.cjs')
    );
...
test('#52 - incorrectly resolves module-paths like "./someFolder/" when there is a file of the same name', function (t) {
    ...
    t.equal(
        resolve.sync('./foo/', { basedir: path.join(dir, 'same_names') }),
        path.join(dir, 'same_names/foo/index.cjs')
    );
    t.end();
});
...
test('sync dot main', function (t) {
    var start = new Date();
    t.equal(resolve.sync('./resolver/dot_main'), path.join(__dirname, 'resolver/dot_main/index.cjs'));
...
test('sync dot slash main', function (t) {
    var start = new Date();
    t.equal(resolve.sync('./resolver/dot_slash_main'), path.join(__dirname, 'resolver/dot_slash_main/index.cjs'));
    t.ok(new Date() - start < 50, 'resolve.sync timedout');
    t.end();
});
